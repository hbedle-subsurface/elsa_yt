#!/usr/bin/env python3
"""
collect.py — gather what people say in YouTube comments about solar
projects proposed near them.

Local TV stations cover county hearings and post the clips. The comments
underneath are written by people who live there, at more length and in
plainer words than a news article reports them.

How it works:

  1. Search for videos, a few phrases per state. YouTube search ignores
     most boolean syntax, so the phrases are short.
  2. Score each video on its title, channel and description. Comments are
     only fetched for videos that look like a local siting story, which
     keeps the junk out and the quota low.
  3. Pull the comments, storing the text, the date and the like count —
     never the author's name or channel.
  4. Refresh everything already on file, because YouTube's developer
     policies require stored API data to be deleted or refreshed inside
     30 calendar days.

When a video falls out of the tracked set, its comment text is removed
and the permalink and your own coding are kept. Your codes are your
research data; the text is YouTube's.

Needs one secret, YOUTUBE_API_KEY. Standard library only.
"""

import argparse
import json
import os
import re
import sys
import time
import urllib.error
import urllib.parse
import urllib.request
from datetime import datetime, timedelta, timezone

HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)
VIDEOS = os.path.join(ROOT, 'data', 'videos.json')
COMMENTS = os.path.join(ROOT, 'data', 'comments.json')
RUNS = os.path.join(ROOT, 'data', 'runs.json')
CODEBOOK = os.path.join(ROOT, 'codebook.json')

API = 'https://www.googleapis.com/youtube/v3/'
KEY = os.environ.get('YOUTUBE_API_KEY', '')

COUNTY_RE = re.compile(
    r'((?:(?:St|Ste|Mt)\.\s+)?[A-Z][a-z]+(?:[ \-][A-Z][a-z]+)?)\s+(?:County|Parish)\b')


# --------------------------------------------------------------- helpers

def load_json(path, default=None):
    try:
        with open(path, encoding='utf-8') as fh:
            return json.load(fh)
    except FileNotFoundError:
        return default


def save_json(path, obj):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, 'w', encoding='utf-8') as fh:
        json.dump(obj, fh, ensure_ascii=False, indent=1)


def now_iso():
    return datetime.now(timezone.utc).isoformat(timespec='seconds')


def today():
    return datetime.now(timezone.utc).strftime('%Y-%m-%d')


def days_since(stamp):
    if not stamp:
        return 9999
    try:
        d = datetime.strptime(stamp[:10], '%Y-%m-%d').replace(tzinfo=timezone.utc)
    except ValueError:
        return 9999
    return (datetime.now(timezone.utc) - d).days


class QuotaExhausted(Exception):
    pass


def api(endpoint, **params):
    params['key'] = KEY
    url = API + endpoint + '?' + urllib.parse.urlencode(params)
    req = urllib.request.Request(url, headers={'Accept': 'application/json'})
    try:
        with urllib.request.urlopen(req, timeout=45) as resp:
            return json.loads(resp.read().decode('utf-8', 'replace')), None
    except urllib.error.HTTPError as exc:
        body = exc.read().decode('utf-8', 'replace')
        reason = ''
        try:
            errs = json.loads(body)['error'].get('errors', [])
            reason = errs[0].get('reason', '') if errs else ''
        except Exception:
            pass
        if reason in ('quotaExceeded', 'dailyLimitExceeded'):
            raise QuotaExhausted(reason)
        return None, '%s %s' % (exc.code, reason or body[:120])
    except Exception as exc:
        return None, '%s: %s' % (type(exc).__name__, exc)


# ------------------------------------------------------------ searching

def build_searches(cfg):
    out = []
    for topic in cfg['topics']:
        states = cfg['states'] if topic.get('per_state') else [None]
        for state in states:
            for phrase in topic['searches']:
                q = phrase + (' ' + state if state else '')
                out.append({'topic': topic['id'], 'state': state, 'q': q})
    return out


def search_videos(q, days, per_search):
    after = (datetime.now(timezone.utc) - timedelta(days=days)
             ).strftime('%Y-%m-%dT%H:%M:%SZ')
    data, err = api('search', part='snippet', q=q, type='video',
                    maxResults=min(50, per_search), order='relevance',
                    regionCode='US', relevanceLanguage='en',
                    publishedAfter=after)
    if err:
        return [], err
    out = []
    for item in data.get('items', []):
        sn = item['snippet']
        out.append({
            'id': item['id']['videoId'],
            'title': sn.get('title', ''),
            'channel': sn.get('channelTitle', ''),
            'description': sn.get('description', ''),
            'published': (sn.get('publishedAt') or '')[:10],
        })
    return out, None


# -------------------------------------------------------------- scoring

def phrases(terms):
    return [t.strip('"').lower() for t in terms if t.strip('"')]


def best_topic(text, cfg, asked):
    matches = [t for t in cfg['topics']
               if any(p in text for p in phrases(t['recognize']))]
    if not matches:
        return None
    for t in matches:
        if t['id'] == asked:
            return t
    return matches[0]


def score_video(video, asked_topic, asked_state, cfg):
    """Whether this looks like local coverage of a siting fight.

    Judged on the topic the title actually matches rather than the search
    that found it, because a search for floating solar turns up ordinary
    solar farms and those are worth keeping too."""
    blob = ' '.join([video['title'], video['channel'], video['description']])
    text = blob.lower()

    spec = best_topic(text, cfg, asked_topic)
    if spec is None:
        # No solar-siting phrase anywhere. Channel boilerplate alone
        # ("subscribe for more news from Payne County, Oklahoma") would
        # otherwise push consumer panel reviews over the threshold.
        return {'score': 0, 'topic': asked_topic, 'counties': [], 'states': [],
                'via_state': asked_state or ''}

    # A phrase in the title is the video's subject; one buried in the
    # description may only be a passing mention.
    score = 3 if any(p in video['title'].lower() for p in phrases(spec['recognize'])) else 2

    hits = sum(1 for a in cfg['angles'] if a in text)
    score += min(hits, 2)

    counties = sorted({m.group(0) for m in COUNTY_RE.finditer(blob)})
    states = [s for s in cfg['states'] if s.lower() in text]
    if counties:
        score += 1
    if states:
        score += 1
    ch = video['channel'].lower()
    if any(h in ch for h in cfg['local_channel_hints']):
        score += 1

    return {
        'score': score,
        'topic': spec['id'] if spec else asked_topic,
        'counties': counties,
        'states': states,
        'via_state': asked_state or '',
    }


def cues_in(text, codebook):
    low = (text or '').lower()
    return [c['id'] for c in codebook['categories']
            if any(q in low for q in c['cues'])]


# ------------------------------------------------------------- comments

def fetch_comments(video_id, max_pages):
    """Comments and their replies, without anything identifying who wrote
    them. Author names are deliberately not requested into storage."""
    out = []
    token = None
    for _ in range(max_pages):
        params = dict(part='snippet,replies', videoId=video_id, maxResults=100,
                      order='relevance', textFormat='plainText')
        if token:
            params['pageToken'] = token
        data, err = api('commentThreads', **params)
        if err:
            # 403 commentsDisabled and 404 videoNotFound are ordinary
            return out, err
        for item in data.get('items', []):
            top = item['snippet']['topLevelComment']
            out.append(one_comment(top, video_id, None))
            for reply in (item.get('replies') or {}).get('comments', []):
                out.append(one_comment(reply, video_id, top['id']))
        token = data.get('nextPageToken')
        if not token:
            break
    return out, None


def one_comment(raw, video_id, parent):
    sn = raw['snippet']
    return {
        'id': raw['id'],
        'video': video_id,
        'text': sn.get('textOriginal') or sn.get('textDisplay') or '',
        'published': (sn.get('publishedAt') or '')[:10],
        'likes': sn.get('likeCount', 0),
        'reply_to': parent,
        'url': 'https://www.youtube.com/watch?v=%s&lc=%s' % (video_id, raw['id']),
    }


# ---------------------------------------------------------------- main

def main():
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument('--states', default='')
    ap.add_argument('--days', type=int, default=0,
                    help='how far back to look for videos')
    ap.add_argument('--topics', default='')
    ap.add_argument('--per-search', type=int, default=25)
    ap.add_argument('--dry-run', action='store_true')
    args = ap.parse_args()

    cfg = load_json(os.path.join(HERE, 'queries.json'))
    codebook = load_json(CODEBOOK)
    if not cfg or not codebook:
        sys.exit('queries.json or codebook.json is missing')
    if args.states.strip():
        cfg['states'] = [s.strip() for s in args.states.split(',') if s.strip()]
    if args.topics.strip():
        want = {t.strip() for t in args.topics.split(',')}
        cfg['topics'] = [t for t in cfg['topics'] if t['id'] in want]

    searches = build_searches(cfg)
    if args.dry_run:
        print('%d searches:' % len(searches))
        for s in searches:
            print('  [%s/%s] %s' % (s['topic'], s['state'] or 'all', s['q']))
        return
    if not KEY:
        sys.exit('YOUTUBE_API_KEY is not set. Add it under Settings, Secrets '
                 'and variables, Actions.')

    days = args.days or cfg.get('search_days', 365)
    threshold = cfg.get('keep_threshold', 3)
    store_v = load_json(VIDEOS, {'videos': []})
    store_c = load_json(COMMENTS, {'comments': []})
    known = {v['id']: v for v in store_v['videos']}
    problems = []
    found_new = 0

    # ---- find videos -------------------------------------------------
    print('%d searches across %d states' % (len(searches), len(cfg['states'])))
    try:
        for i, s in enumerate(searches, 1):
            vids, err = search_videos(s['q'], days, args.per_search)
            if err:
                problems.append('search %s — %s' % (s['q'], err))
                print('  %2d/%d  %-46s failed' % (i, len(searches), s['q'][:46]))
                continue
            kept = 0
            for v in vids:
                marks = score_video(v, s['topic'], s['state'], cfg)
                if marks['score'] < threshold:
                    continue
                kept += 1
                if v['id'] in known:
                    rec = known[v['id']]
                    rec['topics'] = sorted(set(rec.get('topics', []) + [marks['topic']]))
                    rec['score'] = max(rec.get('score', 0), marks['score'])
                    continue
                known[v['id']] = {
                    'id': v['id'],
                    'title': v['title'],
                    'channel': v['channel'],
                    'published': v['published'],
                    'url': 'https://www.youtube.com/watch?v=' + v['id'],
                    'topics': [marks['topic']],
                    'counties': marks['counties'],
                    'states': marks['states'],
                    'via_states': [marks['via_state']] if marks['via_state'] else [],
                    'score': marks['score'],
                    'first_seen': today(),
                    'comments': 0,
                }
                found_new += 1
            print('  %2d/%d  %-46s %2d of %2d kept'
                  % (i, len(searches), s['q'][:46], kept, len(vids)))
    except QuotaExhausted:
        problems.append('search quota exhausted; the rest of the searches were '
                        'skipped. It resets at midnight Pacific.')
        print('  search quota exhausted, moving on to comments')

    # ---- decide what stays in the refresh cycle -----------------------
    videos = sorted(known.values(),
                    key=lambda v: (v.get('published', ''), v.get('score', 0)),
                    reverse=True)
    cap = cfg.get('max_tracked', 250)
    tracked, dropped = videos[:cap], videos[cap:]
    tracked_ids = {v['id'] for v in tracked}

    # ---- fetch and refresh comments ----------------------------------
    by_id = {c['id']: c for c in store_c['comments']}
    pages = cfg.get('max_comment_pages', 5)
    fetched = refreshed = 0

    try:
        for n, v in enumerate(tracked, 1):
            got, err = fetch_comments(v['id'], pages)
            if err:
                if 'commentsDisabled' not in err:
                    problems.append('comments %s — %s' % (v['id'], err))
                v['comments_off'] = True
                continue
            v.pop('comments_off', None)
            v['comments'] = len(got)
            for c in got:
                c['cues'] = cues_in(c['text'], codebook)
                c['refreshed'] = today()
                c['expired'] = False
                if c['id'] in by_id:
                    old = by_id[c['id']]
                    c['first_seen'] = old.get('first_seen', today())
                    refreshed += 1
                else:
                    c['first_seen'] = today()
                    fetched += 1
                by_id[c['id']] = c
            if n % 25 == 0 or n == len(tracked):
                print('  comments: %d of %d videos' % (n, len(tracked)))
    except QuotaExhausted:
        problems.append('comment quota exhausted partway through; anything not '
                        'refreshed this run will be refreshed next run.')
        print('  comment quota exhausted')

    # ---- honor the 30 day rule ---------------------------------------
    # Text not refreshed inside the window is dropped. The permalink stays,
    # so coding done in the page still points at something real.
    expire_after = cfg.get('expire_days', 30)
    expired = 0
    for c in by_id.values():
        if c['video'] not in tracked_ids or days_since(c.get('refreshed')) >= expire_after:
            if not c.get('expired'):
                c['text'] = ''
                c['cues'] = []
                c['expired'] = True
                expired += 1

    comments = sorted(by_id.values(),
                      key=lambda c: (c.get('published', ''), c.get('likes', 0)),
                      reverse=True)

    save_json(VIDEOS, {'updated': now_iso(), 'tracked': len(tracked),
                       'videos': tracked + dropped})
    save_json(COMMENTS, {'updated': now_iso(), 'expire_days': expire_after,
                         'comments': comments})

    runs = load_json(RUNS, {'runs': []})
    runs['runs'].insert(0, {
        'when': now_iso(), 'searches': len(searches), 'states': cfg['states'],
        'new_videos': found_new, 'tracked_videos': len(tracked),
        'new_comments': fetched, 'refreshed_comments': refreshed,
        'expired_comments': expired, 'total_comments': len(comments),
        'problems': problems[:25],
    })
    runs['runs'] = runs['runs'][:60]
    save_json(RUNS, runs)

    live = sum(1 for c in comments if not c.get('expired'))
    print('\n%d new videos, %d tracked' % (found_new, len(tracked)))
    print('%d new comments, %d refreshed, %d expired' % (fetched, refreshed, expired))
    print('%d comments on file, %d with text' % (len(comments), live))
    if problems:
        print('\n%d problems:' % len(problems))
        for p in problems[:10]:
            print('  ' + p)


if __name__ == '__main__':
    main()
